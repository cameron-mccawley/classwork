...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	smallIndex = 0;
	if(rightIndex < max){
		if(compare(getDynArr(heap, leftIndex), getDynArr(heap, rightIndex)) == -1){
			smallIndex = leftIndex;
		}else{
			smallIndex = rightIndex;
		}

		if(compare(getDynArr(heap, smallIndex), getDynArr(heap, pos)) == -1){
			swapDynArr(heap, pos, smallIndex);
			_adjustHeap(heap, max, smallIndex);
		}
...
void removeMinHeap(DynArr *heap)
{
	...
	assert(heap);
	assert(sizeDynArr(heap) > 0);
	heap->data[0] = heap->data[sizeDynArr(heap) - 1];
	heap->size--;
