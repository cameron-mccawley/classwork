...
class IcmpHelperLibrary:
    ...
    class IcmpPacket:
        ...
        #                                                                                                              #
        # ############################################################################################################ #
        def setLost(self, isLost):
            self.lost = isLost
        
        def setCurrentRTT(self, curr):
            self.currentRTT = curr
            ...
            try:
                ...
                if timeLeft <= 0:
                    ...
                    if icmpType == 11:                          # Time Exceeded
                        ...
                        icmpReplyPacket.printResultToConsole(ttl, timeReceived, addr, self)
                        self.setCurrentRTT((timeReceived - pingStartTime) * 1000)
                        self.setLost(False)
    ...
    class IcmpPacket_EchoReply:
        ...
        #adding validators for each part
        ...
        #                                                                                                              #
        # ############################################################################################################ #
        def printResultToConsole(self, ttl, timeReceived, addr, icmp):
            ...
            if(self.isValidResponse()):
                print("Response is \033[92mVALID\033[0m")
            else:
                print("Response is \033[91mINVALID\033[0m")
                
            print("IDENTIFIER:   Expected=%d    Actual=%d   Valid=%r" % (icmp.getPacketIdentifier(), self.getIcmpIdentifier(), self.getIcmpIdentifier_isValid()))
            print("SEQ NUM:      Expected=%d    Actual=%d   Valid=%r" % (icmp.getPacketSequenceNumber(), self.getIcmpSequenceNumber(), self.getIcmpSequenceNumber_isValid()))
            print("RAW DATA:     Expected=%s    Actual=%s   Valid=%r" % (icmp.getDataRaw(), self.getIcmpData(), self.getIcmpRawData_isValid()))
            print()
    ...
    def __sendIcmpEchoRequest(self, host):
        ...
        for i in range(4):
            ...
            icmpPacket.sendEchoRequest(255)                                                # Build IP
            packetsSent += 1
            packetsRecv += 1
            #get packet recv stuff
            icmpPacket.printIcmpPacketHeader_hex() if self.__DEBUG_IcmpHelperLibrary else 0
            icmpPacket.printIcmpPacket_hex() if self.__DEBUG_IcmpHelperLibrary else 0
            totalRTT += icmpPacket.getCurrentRTT()
            if(i == 0):
                minRTT = icmpPacket.getCurrentRTT()
                maxRTT = icmpPacket.getCurrentRTT()
            if(icmpPacket.getCurrentRTT() < minRTT):
                minRTT = icmpPacket.getCurrentRTT()
            if(icmpPacket.getCurrentRTT() > maxRTT):
                maxRTT = icmpPacket.getCurrentRTT()
        
        #print stats
        
        percent = str("{:.2%}".format(1 - packetsSent/packetsRecv))
        print("\033[1m%d packets transmitted, %d received, %s packet loss, time %d ms \033[0m" % (
            packetsSent,
            packetsRecv,
            percent,
            totalRTT
        ...
        print("\033[1mrtt min/avg/max = %.2f/%.2f/%.2f \033[0m" % (minRTT, totalRTT/4.0, maxRTT))
        print()
...
def main():
    ...
    #icmpHelperPing.sendPing("mirror.aarnet.edu.au")
    #icmpHelperPing.sendPing("mirror.as35701.net")
